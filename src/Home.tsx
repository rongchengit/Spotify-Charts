import React, {useState } from 'react'; //{} only grabs useEffect and useState from react
import Graph from './Graph'; // ./ because its local file 
import './App.css';
import {fetchUserPlaylists} from './spotifyUtils';
import {authorizeLogin,getAccessToken,login, logout} from './spotifyLogin'
...
return (
    <div >
        <button onClick={async()=>{await authorizeLogin()}}>login</button> 
        <button onClick={() => logout()}>logout</button> 

      <form className = "center" onSubmit = {handleUserIDSubmit}>
        <input className = "defaultInput" type = "text" name="userID" placeholder ="Username" onChange = {handleUserIDChange}/>
      </form>
      <div>
